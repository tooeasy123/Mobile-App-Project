...
export default function ModifyPeople({ navigation }) {
    ...
    const [employees, setPeopleID] = useState([]);
    //hooks
    useEffect(() => { 
    ...
    useEffect(() => { 
      ...
      fetch("http://192.168.0.4:3000/get_peopledata")
        .then(r => r.json())
        .then(json=>{ 
          let employees = json.map(item => {  return {key:item.PeopleID, value:item.Name }; }); 
          setPeopleID(employees); 
        });
    ...
    const ModifyPeople = () => {
      ...
      let d1 = {Name,Phone,DepartmentID:+DepartmentID,Suburb,Address,State,Postcode,Country,PeopleID:+employees}; 
        ...
        fetch ("http://192.168.0.4:3000/get_UpdatePeopledata", requestOptions)
            .then(r => r.json())
      ...
      return (
          <ScrollView contentContainerStyle={styles.container}>
              ...
              <SelectList setSelected={(val) => {setPeopleID(val)}}data={employees} save="key"/>
              ...
              <Button onPress={ModifyPeople} title="submit New Details" color={"#941a1d"}>Submit New Employee/Staff Details</Button>
          </ScrollView>
      )
